...
class Program
{
    ...
    public void SelectedDomen()
    {
        List<Debtor> debtor = debtors.FindAll(x => x.Email.Contains("rhyta.com") || x.Email.Contains("dayrep.com"));
    ...
    public void NameAndPhoneContent()
    {
        List<Debtor> debtor = new List<Debtor>(debtors);
        foreach (var item in debtor)
        {

            if (item.FullName.Length >= 18 && item.Phone.Contains("7"))
            {
    ...
    public void DebtAverage()
    {
        ...
        Console.WriteLine("This is the ordered list by surname");
        Console.WriteLine(" ");
        foreach (var item in debtor)
        {
            var index = item.FullName.IndexOf(".");
            int first = index + 2;
            //debtor.OrderBy(x => x.FullName[first]);
            Console.WriteLine(item.FullName[first]);
        }

        var orderedDebt = debtor.OrderByDescending(x => x.Debt);
        Console.WriteLine(" ");
        Console.WriteLine("This is ordered list by debt");
        Console.WriteLine(" ");
        foreach (var item in orderedDebt)
        {
            Console.WriteLine(item);
        }
    }

    public void PhoneWithoutEight()
    {
    ...
    public void NameWithThreeSameLetter()
    {
        List<Debtor> debtors = new List<Debtor>();
    ...
    public void BirthYearMax()
    {

    }
    public void TopFiveDebt()
    {
    ...
    public void DebitorsSeenWorldWar()
    {
    ...
    public void PhoneWithUnrepeatedNum()
    {
        List<Debtor> debtor = new List<Debtor>(debtors);
        var count = 0;
        foreach (var item in debtor)
        {
            for (int i = 0; i < item.Phone.Length; i++)
            {
                var ch = item.Phone[i] - 1;
                if (ch == i)
                {
                    count++;
                }
            }
            Console.WriteLine($"{item.Phone}: {count}");
        }
    }
    public void PayUntillBirthday()
    {
        foreach (var item in debtors)
        {
            var debt = item.Debt;
    ...
    static void Main()
    {
        ...
        //debt.DebtLess5000();
        //debt.AgeAverage();
        //debt.NameAndPhoneContent();
        //debt.BornInWinter();
        //debt.DebtAverage();
        //debt.PhoneWithoutEight();
        //debt.NameWithThreeSameLetter();
        //debt.BirthYearMax();
        //debt.TopFiveDebt();
        //debt.TotalDebt();
        //debt.DebitorsSeenWorldWar();
        //debt.PhoneWithUnrepeatedNum();
        //debt.PayUntillBirthday();
        //debt.Smile();
